...
| Task | 目标 | 关键依赖 |
|---|---|---|
| MS07-T01 | 系统表 `__tables` / `__columns` + Schema 页（最大单点） | 无 |
| MS07-T02 | drop_table 接 free-list，物理页释放 | MS07-T01 |
| MS07-T03 | planner.rs 2266 → 按 build_* 拆分到 4-6 个模块 | 无 |
| MS07-T04 | `Database::begin/commit/rollback` 公开 API + 隐式事务向后兼容 | 无 |
| MS07-T05 | Checkpoint 真正工作 | MS07-T01 |
| MS07-T06 | 谓词/LIMIT 下推到 scan 之上 | 无 |
| MS07-T07 | 视 T04/T05 需要决定是否引入消息传递重构 | MS07-T04, MS07-T05 |
