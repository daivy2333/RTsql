...
> 最后更新：2026-09-14（maintainer 工作区保存提交：MS16 收尾改动（2026-09-13 完成、原待提交）与 MS09 实施一并入库——活跃 change `2026-09-13-ms09-engine-mvcc-closeout`（MS09 引擎能力与 MVCC 收尾聚合 change）：Iteration 000 事务可见性域（I033 墓碑抑制 + I032 + Read Committed）三轮 Cycle accepted、Iteration 001 NLJ 与启发式切换两轮 Cycle accepted、Iteration 002 关联子查询缓存 T20-T22 实施完成 Act reported 待 Plan Review；全量 936 tests / 0 failed / 2 ignored，clippy/fmt/validate 全 0/PASS）
> 同步状态：current
...
- **分支**: master
- **最新 revision**: d8a244f（MS15 close-out 实施 commit；其后 e51c4a3 docs 提交 + 2026-09-14 保存提交——MS16 收尾与 MS09 实施入库，保存提交 hash 由下次 docs sync 记录）
- **ahead of origin**: 21 commits
- **工作区**: 2026-09-14 保存提交统一入库——MS16 收尾（src/tests + change 归档 + specs 27 + improvements I046/I047 promoted + tasks/SNAPSHOT 同步）+ MS09 change `2026-09-13-ms09-engine-mvcc-closeout` 全部规划产物与 Iteration 000/001/002 实施（含新 `src/executor/nested_loop_join.rs`、`tests/mvcc_tombstone_visibility_test.rs` / `isolation_level_test.rs` / `nested_loop_join_test.rs`、`.claude/issues/` ISS01/ISS02）
- **最新 tag**: M11
- **测试**: 936 tests pass, 0 failures, 2 ignored（2026-09-14 MS09 Iteration 002 Cycle 000 Act 运行；= MS16 收尾 892 基线 + MS09 新增套件 mvcc_tombstone_visibility / isolation_level / nested_loop_join + subquery_test 缓存见证 8 用例；2 ignored 为信号标定设计项）
- **OpenSpec**: 27 capability specs + 1 活跃 change validate PASS（`validate --specs --changes` 28 items；活跃 change `2026-09-13-ms09-engine-mvcc-closeout`——Iteration 000/001 accepted，002 Act reported 待 Plan Review）

## 同步状态

- `current` — 文档与代码一致（2026-09-14 保存提交统一入库 MS16 收尾与 MS09 实施至 Iteration 002 Act reported；SNAPSHOT 已增量刷新，保存提交 hash 待下次 docs sync 记录）
...
- 变更: `openspec/changes/`（活跃：`2026-09-13-ms09-engine-mvcc-closeout`——MS09 引擎能力与 MVCC 收尾聚合 change，4 delta specs + 3 Iteration，000/001 accepted、002 Act reported；归档目录含 MS06-T01 + MS06-T02 + MS06-T03-T04 + MS07-T01 + MS07-T02 + MS07-T03 + ms07-rest + ms08-t01-t02 + ms10-t01-cli-shell + ms10-t02-file-lock-graceful-shutdown + ms10-t03-file-format-header + ms10-t04-multi-statement-execution + ms10-t05-lifecycle-subcommands + ms11-t01-sql-expressions + ms11-t02-sql-transaction-statements + ms11-t03-scalar-functions carrier + ms15-t01-keyless-eq-routing carrier + ms15-rest-correctness-batch carrier + ms16-correctness-batch carrier）
...
- `openspec/specs/table-name-resolution/spec.md`（MS15-Rest 落地，3 个 Requirement：R1 标识符表名解析归一化 / R2 dump 与 schema 表名保真 / R3 既有裸名语义零回归）
