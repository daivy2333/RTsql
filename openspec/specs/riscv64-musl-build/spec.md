# riscv64-musl-build Specification

## Purpose
定义从 x86_64 或其他兼容宿主机生成 RISC-V 64 Linux musl 数据库二进制与可传递归档的前置检查、输出契约和验证边界，使目标架构不可执行时仍能完成可复现的构建交付。来源：MS17 后续分发扩展 change `2026-09-24-riscv64-musl-build-artifacts`。
