...
约束单表 SELECT 键位等值过滤的 planner 路由正确性：索引路由（IndexScan 点查 / Scan 索引遍历）仅当其结果覆盖表中全部可见行时可用；键位不可键控的行（MS10-T05 001-rework 语义：落库不入索引）必须经数据页行内求值路径可达。来源：MS15-T01（change `2026-09-12-ms15-t01-keyless-eq-routing`，2026-09-12 归档，improvements I036）；MS16 扩展键列类型感知路由（change `2026-09-12-ms16-correctness-batch`，2026-09-13 归档，improvements I046 形态 2 + 调查新发现）。
