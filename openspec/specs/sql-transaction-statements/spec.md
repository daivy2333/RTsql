...
## Purpose
SQL 事务语句能力。`BEGIN`（含 `BEGIN TRANSACTION`/`START TRANSACTION`，不带 mode）、`COMMIT`、`ROLLBACK` 在 CLI one-shot 调用内驱动会话事务态：事务开启后普通语句经 `execute_stage_in_tx` 在该事务内执行（不再逐条 auto-commit），语义等价 lib 显式事务 API（MS07-T04）；事务语句成功响应为受影响行数 0（与 DDL 同构渲染）。边界子句（`SET TRANSACTION`/`SAVEPOINT`/`RELEASE SAVEPOINT`/带 mode 的 BEGIN/`AND CHAIN`/`ROLLBACK TO SAVEPOINT`）显式拒绝并点名；lib 非会话路径（`execute_sql`/`execute_in_tx`/网络同源）精确拒绝且不进计划缓存。事务内 SELECT 与 DDL 沿引擎既有无快照扫描语义（文档化，不改变）。来源：MS11-T02（change `2026-09-10-ms11-t02-sql-transaction-statements`，2026-09-10 归档）。
