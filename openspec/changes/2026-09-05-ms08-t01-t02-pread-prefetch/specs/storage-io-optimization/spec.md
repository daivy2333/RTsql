...
MS08-T01+T02：`FileStorage` 页 I/O 位置参数化（pread/pwrite，消除每页 lseek syscall 与共享偏移竞态）；`DataScanExecutor` 数据页链扫描双缓冲预取（下一页加载与当前页行处理重叠）。两个 Iteration 独立验收，查询结果与错误语义零变化。
