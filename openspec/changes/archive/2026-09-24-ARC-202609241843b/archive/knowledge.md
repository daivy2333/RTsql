# 归档载体：knowledge 域全文（ARC-202609241843b 批次）

> 本文件是 openspec-archivist 清理批次的 Archive 载体，保存 knowledge spec 退役时 K01-K38 的完整原文（逐字，含原分节）。
> 条目原位置：`openspec/specs/knowledge/spec.md`；退役日期：2026-09-24（用户指令；迁移去向见 proposal 映射表）。
