# 归档载体：decisions 域全文（ARC-202609241843b 批次）

> 本文件是 openspec-archivist 清理批次的 Archive 载体，保存 decisions spec 退役时 D01-D12 的完整原文（逐字）。
> 条目原位置：`openspec/specs/decisions/spec.md`；退役日期：2026-09-24（用户指令：现行 CLAUDE.md 信息路由不再设决策域，长期选择及其理由由 change design.md 承载）。
