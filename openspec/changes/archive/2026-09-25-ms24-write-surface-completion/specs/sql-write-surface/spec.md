...
定义 SQL 写入面的补全语义：子集列清单 INSERT 与 DEFAULT 的解析、持久化与应用闭环；写入值与列声明类型的强制一致门（ISS04）；UPSERT（ON CONFLICT DO NOTHING / DO UPDATE）与 REPLACE INTO 的冲突仲裁与原位更新语义；UPSERT 冲突目标与不支持形态的计划期点名拒绝；既有写面语义零回归。
