# planner-module-decomposition

## Purpose

将单文件 `src/parser/planner.rs`（2266 行）按职责拆分为 `src/parser/planner/` 目录下六个模块，使 `PlanBuilder` 的 `public` API 与 SQL 计划生成行为完全不变，同时每个子模块可独立单测。这是纯重构：函数正文逐字保留，只搬移位置并做最小可见性调整。
