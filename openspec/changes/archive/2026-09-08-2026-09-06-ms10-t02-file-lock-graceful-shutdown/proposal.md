...
> **Review 裁定（2026-09-06，Iteration 000 Cycle 000-initial Plan Review，`replan-required`）**：T0（reader 逐帧无歧义）完成且经独立审查通过；T1 被 `test_sigkill_leaves_recoverable_db` 暴露的**引擎级恢复正确性缺陷**阻塞（驱逐规模 + 未 checkpoint WAL 的重开：10k 行实测 13190 ≠ 10000，checkpoint 后仍损坏；160k 档同机制 +3190）。Plan 独立复现 + 代码链调查裁定根因为 `recovery.rs::redo_record` **追加式逻辑重放非幂等**（忽略记录自带 `row_id`；叠加 stale catalog tail 恢复 → 链分叉；同族缺口：重放不重建 PK 索引、不重放 Delete 墓碑、Update 重放丢 `next_version` 链）。修复属 replan 级设计决策（Act 的 Forbidden 面外的 recovery/data_page 层），并入本 change Iteration 000 作为 **Replan Cycle 001**（新全局任务 T0b，新 delta spec `wal-recovery-replay-integrity`，design D7）。关键设计事实：`Update` 记录的 `row_id` 本就是新版本位置（`update.rs:117`），修复**零 WAL 格式变更、零页格式变更**。本 note 之下的原始范围叙述不变；锁/停机（Iteration 001）不受影响。
